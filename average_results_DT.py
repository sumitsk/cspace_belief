...
if __name__ == '__main__':
    ...
    avg_acc = np.zeros(len(N_values)) # , len(r_values))) 
    avg_err = np.zeros(len(N_values)) # , len(r_values)))
    avg_accw = np.zeros(len(N_values)) # , len(r_values)))
    avg_errw = np.zeros(len(N_values)) # , len(r_values)))
    ...
    np.savez(os.path.join(results_path, 'avg_DT'),
             acc=avg_acc,
             accw=avg_accw,
             err=avg_err,
             errw=avg_errw,
